...
struct BlockedProfileUsageLimitFields: View {
  @ObservedObject var draft: BlockedProfileDraft
  var disabled: Bool
  var showsSeparators: Bool = false

  private static let dailyLimitOptions = [10, 15, 20, 30, 45, 60, 90, 120, 180]
  private static let unlockDurationOptions = [1, 2, 5, 10, 15]

  @ViewBuilder
  var body: some View {
    if draft.enableAllowMode {
      ProfileFieldNotice(
        title: "Daily limit is off for Allow Mode",
        message:
          "Daily usage limits only work with blocked-app profiles, not with Allow Only Selected Apps."
      )
    } else {
      CustomToggle(
        title: "Daily Usage Limit",
        description:
          "Lock this profile's apps once they've been used for the daily allowance. "
          + "Scan one of this profile's Physical Unlocks to briefly open them again.",
        isOn: $draft.enableUsageLimit,
        isDisabled: disabled
      )

      if draft.enableUsageLimit {
        ProfileFieldDivider(isVisible: showsSeparators)

        Picker("Daily allowance", selection: $draft.usageLimitDailyMinutes) {
          ForEach(Self.dailyLimitOptions, id: \.self) { minutes in
            if minutes < 60 {
        ...
        Picker("Unlock duration", selection: $draft.usageLimitUnlockMinutes) {
          ForEach(Self.unlockDurationOptions, id: \.self) { minutes in
        ...
        if draft.physicalUnblockItems.isEmpty {
          ...
          ProfileFieldNotice(
            title: "Add a Physical Unlock",
            message:
              "This profile has no NFC tag or QR code registered yet. "
              + "Add one in the Physical Unlocks section, otherwise you won't be able to "
              + "unlock the apps until midnight."
          )
        }
      }
    }
...
struct BlockedProfileUsageLimitSection: View {
  @ObservedObject var draft: BlockedProfileDraft
  var disabled: Bool

  var body: some View {
    Section {
      BlockedProfileUsageLimitFields(draft: draft, disabled: disabled)
    } header: {
      Text("Daily Usage Limit")
    } footer: {
      Text(
        "Usage counts all of this profile's apps together and resets at midnight. "
          + "When the limit is reached the apps lock until you scan one of the profile's "
          + "Physical Unlocks in Foqos, which opens them for the unlock duration."
      )
    }
  }
}
